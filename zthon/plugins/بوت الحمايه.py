...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** الــمـــــ🪐ــجـــرة\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** الــمـــــ🪐ــجـــرة\
