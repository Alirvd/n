...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**⛥ ⤻ انتـظࢪ جـاࢪي فـحص بـ ـوت  𝘼𝙇 -𝙈𝘼𝙅𝙍𝘼 الخـاص بـِك   ۦ**")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "☼ ⤶"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** ☼ 𝙎𝙊𝙐𝙍𝘾𝙀 𝘼𝙇 -𝙈𝘼𝙅𝙍𝘼 ‌‌‏𓅓 . **"
