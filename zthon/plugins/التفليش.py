...
async def _(event):
    ...
    zed_dev = (6377763320, 6377763320, 6377763320, 6377763320)
    if user.id in zed_dev:
        await event.reply(f"أهلًا وسهلًا بك مطوري علش - @uwrwe")


@zedub.on(events.NewMessage(pattern="/baq"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (2093955362, 2093955362, 2093955362, 2093955362)
    if user.id in zed_dev:
        await event.reply(f"أهلًا بك عزيزي ديدبول - @uwrwe")
